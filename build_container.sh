...
# Get the real IP address of the host
get_real_ip() {
...
# Replace 127.0.0.1 with the real IP address if present
if [[ "$PROXY_URL" == *"127.0.0.1"* ]]; then
    REAL_IP=$(get_real_ip)
    PROXY_URL=$(echo "$PROXY_URL" | sed "s/127.0.0.1/$REAL_IP/")
    echo "Replaced 127.0.0.1 with real IP: $REAL_IP"
fi
